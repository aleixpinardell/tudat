...
BOOST_AUTO_TEST_CASE( third_body )
{
    ...
    std::vector< boost::shared_ptr< DSSTForceModel > > forceModels;
    for ( unsigned int i = 0; i < 1; i++ ) { // for each perturbation
        DSSTThirdBody forceModel( aux, thirdBody );
        forceModels.push_back( boost::make_shared< DSSTThirdBody >( forceModel ) );
    }
    ...
    for ( double epoch : integrationEpochs ) { // for each integration step
        ...
        for ( auto forceModel : forceModels ) {
            meanElementRate += forceModel->getMeanElementRates();
        }
    ...
    std::cout << meanElementRates[ initialEpoch ] << std::endl;
}
